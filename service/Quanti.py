...
def test_iqa(parameters, original_path, result_path,csvname):
    ...
    with open(os.path.join(Util.unify_path().user_path,csvname), 'a', encoding='utf-8', newline='') as f:
...
def get_loss(progress_signal,index_num,inputf,outputf,row,csvname):
    input_dir = inputf
    output_dir = outputf
    print(outputf,"+++++++",sep='')
    ...
    for i,file in enumerate(files):
        if not os.path.isdir(file):
            ...
            test_iqa(parameters, original_path, result_path,csvname)
            progress_percentage = int((i + 1) / total_files * 100)
            progress_signal.emit(progress_percentage, 'green',1,row,0.0,9999999)
            print('End')

def quant(progress_signal,alg_index_num,row,subid,inputf,outputf):
    f = open(os.path.join(Util.unify_path().user_path,f'quant{row}_{subid}.csv'), 'a', encoding='utf-8', newline='')
    csvname = f'quant{row}_{subid}.csv'
    csv_writer = csv.writer(f)
