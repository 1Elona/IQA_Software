...
class Ui_Dialog(object):
    # 定义两个信号用于窗口切换
    switch_window_to_login = QtCore.pyqtSignal(str)
    switch_window_to_regist = QtCore.pyqtSignal()
    ...
    def setupUi(self, Dialog):
        ...
        # 根据运行环境设置路径
        exe_path = util.Util.unify_path().user_path
    ...
    def login(self):
        ...
        if Database.login(username, password):
            self.switch_window_to_login.emit(username)
        else:
    ...
    def show_regist(self):
        # 1打开新窗口
        self.switch_window_to_regist.emit()
